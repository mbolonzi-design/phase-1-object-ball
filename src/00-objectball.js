...
function playerNumbers(team) {
    ...
    if (homeTeam().teamName === team) {
        for(let player in homePlayers()) {
            numbersArray.push(homePlayers()[player].number)
        }
    } else if(awayTeam().teamName === team) {
        for(let player in awayPlayers()) {
            numbersArray.push(awayPlayers()[player].number)
        }
    }
...
function bigShoeRebounds() {
    ...
    const bigShoe = Math.max(...shoeArray)
    for(let playe in players()) {
        if(players()[playe].shoe === bigShoe) {
            return players()[playe].rebounds
        }
    }
}
